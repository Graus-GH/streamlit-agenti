...
/* Primary button brand (se usato altrove) */
...
/* Pulsanti più compatti in generale */
.stButton > button, .stDownloadButton > button { padding: 6px 10px; line-height: 1; }
/* Riduci gap orizzontale tra colonne dei pulsanti */
...
for k in ["res_select_all_toggle", "basket_select_all_toggle", "reset_res_selection",
          "reset_basket_selection", "flash", "include_fw", "prefer_basket", "prefer_search"]:
...
with st.sidebar:
    ...
    with st.form("search_form_sidebar", clear_on_submit=False):
        ...
        # Checkbox Fine Wines con nota nel box arancione (su due righe)
        st.checkbox(
        ...
        # Min (riga 1) -> Max (riga 2) -> Slider (riga 3)
        ...
        if submitted_sidebar:
            # apri il tab Ricerca dopo submit
...
with tab_basket:
    ...
    # 1) Seleziona/Deseleziona tutto (senza rerun: non serve)
